...
from numpy import apply_along_axis, arange, asarray, divide, dot, empty, euler_gamma, flatnonzero, full, int64, log,\
                  logical_or, ndarray, split, sum, where, zeros_like
...
class VoronoiITree:
	...
	def recursive_depth_search(self, node: VoronoiINode, data: ndarray, depths: ndarray) -> ndarray:
		...
		if node.split_points is None or data.shape[0] == 0:
	...
	def recursive_path_search(self, node: VoronoiINode, data: ndarray, paths: ndarray) -> ndarray:
		...
		if node.split_points is not None and data.shape[0] != 0:
			# Partition data
			partition_indices: list = self.split_data(data, node.split_points)
			# Fill the vector of leaves index
			for i, indices in enumerate(partition_indices):
	...
	def split_data(self, data: ndarray, split_points: ndarray) -> list:
		...
		membership[arange(full_membership.shape[0]),
				   apply_along_axis(lambda x: choice(flatnonzero(x)), 1, full_membership)]: ndarray = True
	...
	def tanimoto_distance(XA: ndarray, XB: ndarray) -> ndarray:
		...
		num: ndarray = AB
		denom: ndarray = A_squared[:, None] + B_squared[None, :] - AB
		Y: ndarray = divide(num, denom, out=zeros_like(num, dtype=float), where=logical_or(num != 0, denom != 0))
